...
# ===== BACKGROUND IMAGE =====
bg_image = Image.open("bgg.png")   # background image path
bg_image = bg_image.resize((1280, 720), Image.LANCZOS)
bg_photo = ImageTk.PhotoImage(bg_image)

bg_label = tk.Label(window, image=bg_photo)
bg_label.image = bg_photo   # reference save (IMPORTANT)
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                root.configure(background='grey80')
                cs = os.path.join(SCRIPT_DIR, fileName)
    ...
    def show_attendance_popup(csv_path, title="Attendance"):
        popup = tk.Toplevel(windo)
        popup.title(title)
        ...
        with open(csv_path, newline="", encoding='utf-8') as file:
            ...
            for col in reader:
                c = 0
                for row in col:
                    label = tk.Label(frame, width=12, height=1, fg="black", font=('times', 12, 'bold'),
    ...
    def Attf():
        ...
        csv_files = []
        for root_dir, dirs, files in os.walk(attendance_dir):
            for f in files:
                if f.endswith('.csv'):
        ...
        if not csv_files:
            ...
            popup.configure(background='grey80')
            tk.Label(popup, text="No attendance sheets found.", fg="black", bg="grey80",
                     font=('times', 14)).pack(pady=20)
            tk.Button(popup, text="OK", command=popup.destroy, fg="black", bg="SkyBlue1").pack()
            return
        csv_files.sort(key=os.path.getmtime, reverse=True)
        picker = tk.Toplevel(windo)
        picker.title("Select Attendance Sheet")
        picker.geometry('500x300')
        picker.configure(background='grey80')
        tk.Label(picker, text="Select a file to view:", fg="black", bg="grey80",
                 font=('times', 12, 'bold')).pack(pady=10)
        listbox = tk.Listbox(picker, width=60, height=10, font=('times', 11))
        ...
        def view_selected():
            ...
            if sel:
                csv_path = csv_files[sel[0]]
                picker.destroy()
                show_attendance_popup(csv_path, "Attendance - " + os.path.basename(csv_path))

        tk.Button(picker, text="View", command=view_selected, fg="black", bg="SkyBlue1",
                  width=15, font=('times', 12, 'bold')).pack(pady=10)
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == 'anup':
            if password == 'anup123':
                win.destroy()
                cs = os.path.join(SCRIPT_DIR, 'StudentDetails', 'StudentDetails.csv')
                if not os.path.exists(cs):
                    err_win = tk.Tk()
                    err_win.title("Error")
                    err_win.geometry('400x100')
                    err_win.configure(background='grey80')
                    tk.Label(err_win, text="No registered students found.", fg="red", bg="grey80",
                             font=('times', 14, 'bold')).pack(pady=20)
                    tk.Button(err_win, text="OK", command=err_win.destroy, fg="black", bg="SkyBlue1").pack()
                    return
                root = tk.Tk()
                root.title("Student Details")
                root.configure(background='grey80')
                with open(cs, newline="", encoding='utf-8') as file:
                    reader = csv.reader(file)
                    r = 0
...
message = tk.Label(window, text="AI Based Face Recognition Attendance System", bg="black", fg="white", width=50,
